 ...
 * Status: PASS
 * Reason: same bitfield-generator value-constructor pattern as an earlier fix
 * (bitfield_value_constructor.c), but reached through a struct
 * field instead of a bare identifier -- `fc_ret.remainder = cap_new(...)`
 * where `fc_ret` is a plain (non-pointer) local, never dereferenced,
 * indexed or NULL-checked. An earlier fix's pointer-evidence guard only ever
 ...
 * still flagged a leak post-580 (an earlier fix's delta-adjudication, 45 sel4
