...
impl Dcl31C {
    ...
    fn track_function_declaration(&self, node: &Node, source: &str) {
        ...
        if node.kind() == "function_definition" {
            ...
            for name in self.typedef_function_pointer_param_names(node, source) {
                self.declared_functions.borrow_mut().insert(name);
            }
        }
...
impl CertRule for Dcl31C {
    ...
    fn set_project_context(&self, context: &ProjectContext) {
        ...
        *self.function_pointer_typedef_names.borrow_mut() =
            context.function_pointer_typedef_names.clone();
...
impl Dcl31C {
    /// Names of parameters whose declared type is a typedef that resolves
    /// (via the shared [`resolve_typedef_chain`]) to a function-pointer
    /// typedef. Motivating case: sqlite's `static void f(RecordCompare
    ...
    fn typedef_function_pointer_param_names(&self, func_node: &Node, source: &str) -> Vec<String> {
        let mut names = Vec::new();
        let fp_typedefs = self.function_pointer_typedef_names.borrow();
        if fp_typedefs.is_empty() {
            return names;
        }
        ...
        for i in 0..param_list.child_count() {
            ...
            // Only a bare typedef spelling is in scope here -- a longhand
            // function-pointer declarator is already caught by
            ...
            if !fp_typedefs.contains(&resolved) {
                continue;
            }
            let Some(declarator) = param.child_by_field_name("declarator") else {
                continue;
            };
            if let Some(name) = ast_utils::find_identifier_in_declarator(&declarator, source) {
                names.push(name);
            }
        }
        names
