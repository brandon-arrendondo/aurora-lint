...
pub struct Exp34C {
    ...
    prescan_global_var_states: RefCell<Arc<HashMap<String, NullState>>>,
...
fn is_unsafe_at(
    ...
    // being reassigned between the assert and the dereference. Only when the
    // policy credits an NDEBUG-strippable assert (`assert_is_guard`).
    if analysis.policy.assert_is_guard
        && is_guarded_by_precondition_assert(var_name, deref_node, source)
    {
        return false;
    }
