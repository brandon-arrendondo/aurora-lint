...
.. note::
   ...
   **PARTLY RESOLVED** — ``python -m bench realworld-run --tool infer`` and
   ``--tool frama-c`` now work against all nine corpora.
...
.. note::
   ...
   **TODO** — run 215's aurora-lint rows for ``hostap`` and ``sqlite`` both record
   ...
   the 490.9 s would be wrong in aurora-lint's favor, which is worse. Tracked
   as a benchmarking_db follow-up.
...
.. note::
   ...
   (rows present) and ``cwes_measured``. Tracked separately for both counts.
...
.. note::
   ...
   **TODO** — this section cannot be written yet.
   ...
   adjudicating a second tool at the same coordinate would create).
...
.. note::
   ...
   **All four competitor columns were re-measured on 2026-09-04**, on one
   ...
   of single-date rather than five months from it.  Tracked as a follow-up.
