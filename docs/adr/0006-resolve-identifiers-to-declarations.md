...
- Any rule consuming `struct_field_types` or `typedef_types` has this
  exposure, not just the one where it was found. If a rule merges a
  project-wide by-name map with the current file's own definitions, check
  which side wins — the file's must.
