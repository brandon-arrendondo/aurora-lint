...
| `arm_assumptions` / `line_compiles_under` | `(node: &Node, source: &str) -> Vec<(String, bool)>` / `(source: &str, line: usize, assumptions: &[(String, bool)]) -> bool` | The macro states a node's enclosing arms fix (`#ifdef`/`#ifndef`/`defined()`/`!defined()` terms, `&&` conjunctions, the `#else` of a single-term condition; include guards skipped), and whether a line of another file survives preprocessing with those states fixed. Pairs a definition with the callers it can compile with (ADR-0010 D4): hostap's `eapol_sm_init` stub frees `ctx` in the `#else` of `IEEE8021X_EAPOL`, and `preauth.c` calls it only under `defined(IEEE8021X_EAPOL)`. Carried per freed index in `FunctionSummary::free_arms` and read through `FunctionSummary::frees_at` / `unconditional_frees_at` (MEM30-C, MEM31-C). Name resolution: it picks which definition a call can link against, never whether a finding is emitted. |
