...
| `condition_excludes_zero` | `(condition: &Node, exprs: &[String], known_true: bool, source: &str, macros: &MacroConstantMap) -> bool` | What a resolved condition PROVED about zero, where `condition_compares_var` only says a variable was tested: given the branch from `dominating_condition_branch`, whether the condition rules out zero for any of `exprs` (matched on whitespace-stripped text via `squeeze_text`, so `s->n`, `f(x)` and `( a - b )` all work). Decomposes a true `&&` / false `\|\|`, flips through `!`, and reads a leaf `x OP c` against a const-evaluated `c` with the operator normalised (x on the left) and negated on a false branch: `x != 0`, `x == c` (c≠0), `x > c` (c≥0), `x >= c` (c≥1), `x < c` (c≤0), `x <= c` (c≤-1), or bare truthy `x`. A true `\|\|`, a false `&&`, and a non-constant bound prove nothing. INT33-C is the reference caller (task 1274: `n ? a / n : 0`, `n > 0 && a / n`, `n == 0 \|\| a / n`, `if (n == 0) break;`). |
