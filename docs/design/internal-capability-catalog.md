...
| `subtree_dereferences_var` | `(node: &Node, var: &str, source: &str) -> bool` | Whether `node`'s subtree dereferences `var` -- `var->f`, `*var`, `var[i]` -- asked of the AST, so neither `var.f` nor a declarator (`char **var = ...`, a `pointer_declarator`) counts. The body-text `contains("*var")` checks in `function_summary` cannot tell a declaration from a dereference; use this when the name is a local the body itself declares. `function_summary::cast_alias_dereferenced` asks it of a local cast from a `void *` parameter. |
