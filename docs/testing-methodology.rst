...
``MSC25-C`` and ``ENV04-C`` have no detection logic at all — no ``.rs``
file exists; see the "Tracked but not implemented" section of
:doc:`configuration` for why they are unimplemented. ``FLP01-C`` does have
one (``flp01_c.rs``), but CERT itself marks the rule "unenforceable"
through automated analysis, and the implementation is a deliberate
conservative stub whose ``check()`` unconditionally returns no violations
— so it is not merely missing a fixture, it has nothing a fixture could
trigger. A fixture is not the missing piece for any of the four.
