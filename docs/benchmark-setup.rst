...
The setup playbook does not fetch Juliet; it only creates the parent
directory and reports whether the suite is already in place. The `NIST
Juliet Test Suite v1.3 <https://samate.nist.gov/SARD/test-suites/112>`_
page links a single zip that downloads directly, with no click-through.
NIST does not promise that URL stays put, so if the ``curl`` below fails,
start from that page. The archive extracts to ``C/``, which is renamed to
the directory name ``bench/config.py`` expects:
...
.. code-block:: bash
    ...
    mv C juliet-test-suite-c
    # Expected structure:
    #   juliet-test-suite-c/testcases/CWE*/     (118 CWE directories)
    #   juliet-test-suite-c/testcasesupport/    (shared helper functions)

The per-CWE manifests that fast mode runs (``rules_templates/cwe/*.toml``)
are generated by ``scripts/generate_rule_cwe_map.py`` and are not
committed, so that directory is nearly empty in a fresh clone.
